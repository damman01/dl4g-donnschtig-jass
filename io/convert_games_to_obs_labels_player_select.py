...
def generate_logs(filename, output: str, output_dir: str, max_entries_per_file: int, shuffle: bool, player_dir: str):
    """
    ...
    top_players = list()
    with open(player_dir, mode='r') as csvfile:
        for line in csvfile:
            top_players.append(int(line.strip()))
        ...
        #for filename in files:
            ...
            with open(filename, mode='r') as file:
                for line in file:
                    ...
                    nr_entries_read += 1
                    if any(player_id in top_players for player_id in entry_game_log.player_ids):
                        #
...
def generate_logs_trump(filename, output: str, output_dir: str, max_entries_per_file: int, shuffle: bool, player_dir: str):
    """
    ...
    Args:
        ...
        max_entries_per_file: maximal number of entries in a file
        shuffle: shuffle entries before writing
        player_dir: directory with player files
...
def main():
    ...
    parser.add_argument('--player_dir', type=str, help='file with playerlist', default='.')
    ...
    if args.recursive:
        ...
        search_path = os.path.join(args.files[0], '**/*.txt')
    ...
    for file in files:
        ...
        os.makedirs(output_dir, exist_ok=True)
        print('Writing files to: {}'.format(output_dir))

        if args.trump:
            generate_logs_trump(file, "trump_" + filename + "_part_", output_dir, args.max_entry, args.shuffle, args.player_dir)
        else:
            generate_logs(file, filename + "_part_", output_dir, args.max_entry, args.shuffle, args.player_dir)
